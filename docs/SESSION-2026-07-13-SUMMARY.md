...
- **Windows-track VERIFIED — NO defect (checked TT-6-D, recorded so it isn't re-investigated):** the shipped Windows Student **does** filter targeted commands by `IsForMe` — the check lives in the **Service** (`ClassroomCtrl.Student.Service/ClassroomWorker.cs:352`, applied per-command `if (!IsForMe(env)) return;`). So the wrong-blast-radius bug found in TT-6-D was a **macOS-port omission** (the port collapsed Service+Agent into one process and dropped the filter), **not** a shipped defect — customer A (Windows) was never exposed. Fixed in the port at `StudentEnvelopeFilter.IsForMe` (guarded in `ConnectionViewModel.Dispatch`).
