...
| **11.2.9** | Recording teacher screen **+ audio** (incl. "Share Computer Audio" / system audio) | ✅ **first-party — CONFIRMED 2026-07-14** | SCK `capturesAudio` captures system audio (non-silent) AND coexists with screen in one SCStream — no third-party device. TCC = Screen Recording (the TT-8 grant). Still ask the customer if "เสียงของครู" = mic or system audio (both now unblocked). See `docs/TT-10-SYSTEM-AUDIO-PROBE.md`. **Capture proven + TT-10-C "Share Computer Audio" BUILT** (system audio → AudioStreamFrame broadcast, no wire change; LIVE-pending). |
