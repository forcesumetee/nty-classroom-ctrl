...
| (audio) | "Share Computer Audio" (system audio) | 🟡 **very likely first-party; unconfirmed** | SCK `capturesAudio` API is real + starts with no third-party device (rules out ❌ BlackHole); headless probe got 0 buffers → needs a foreground-app test on a Mac. See `docs/TT-10-SYSTEM-AUDIO-PROBE.md`. |
