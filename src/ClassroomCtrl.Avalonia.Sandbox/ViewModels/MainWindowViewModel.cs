...
public class MainWindowViewModel
{
    ...
    /// <summary>Phase 25.4 — host VM for the ported StudentCard + its ContextMenu.</summary>
    public StudentGridDemoViewModel StudentGrid { get; } = new();
    ...
    /// <summary>Phase 25.7-B — VM for the ported CameraSelectorDialog.</summary>
    public CameraSelectorDemoViewModel CameraSelector { get; } = new();

    /// <summary>Phase 25.7-C — VM for the ported TeacherIPDialog (first Student-side view).</summary>
    public TeacherIpDemoViewModel TeacherIp { get; } = new();

    /// <summary>Phase 26.0 — live wire connection to a real/mock Teacher.</summary>
    public ConnectionViewModel Connection { get; } = new();

    /// <summary>Phase 27-A — ScreenCaptureKit capture (permission flow + capture).</summary>
    public ScreenCaptureViewModel ScreenCapture { get; } = new();

    /// <summary>Phase 28-C — AVCaptureSession camera capture (permission + device
    /// selection + live JPEG self-preview).</summary>
    public CameraCaptureViewModel CameraCapture { get; } = new();

    /// <summary>Phase 29-D — AVAudioEngine mic capture (permission + live RMS meter).</summary>
    public AudioCaptureViewModel AudioCapture { get; } = new();
